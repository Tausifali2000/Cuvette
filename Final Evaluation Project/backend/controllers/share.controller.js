...
export const shareWorkspace = async (req, res) => {
  try {
    ...
    const existingAccess = workspace.accessList.find(access => access.email === emailToShareWith);
    if (existingAccess) {
      return res.status(400).json({ success: false, message: "User already has access to this workspace" });
    }
